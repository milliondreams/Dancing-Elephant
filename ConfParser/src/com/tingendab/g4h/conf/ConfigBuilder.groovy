...
class ConfigBuilder extends BuilderSupport {
				...
				default:
					...
					if(node instanceof java.util.Map){
						if(node.name == INPUTNAME){
							...
							if(node.value.format == null) node.value.format = "FileOutputFormat"
	...
	public void processJobs(String appName){
		...
		if(reducerName != null || reducerName == ""){
			try{
				...
				if(doesExtend(clazz,"org.apache.hadoop.mapreduce.Reducer")){
					...
					j.getConfiguration().set("g4h.combiner", reducerName)
					j.setReducerClass GroovyCombiner.class
				}
			}catch(ClassNotFoundException cnf){
				j.getConfiguration().set("g4h.combiner", reducerName)
				j.setReducerClass GroovyCombiner.class
